...
document.getElementById("phone-nav-btn").addEventListener("click", () => {
    ...
    timeline3
        .to("#phone-nav", { left: "0%", duration: 0.2 })
        .from(".animp1", { opacity: 0, y: "-50", stagger: 0.1, duration: 0.2 });
});

document.getElementById("cross").addEventListener("click", () => {
    var timeline3 = gsap.timeline();
    timeline3
        .to(".animp1", { opacity: 0, y: "-50", stagger: 0.1, duration: 0.2 })
...
function showCodimg() {
    ...
    for (let i = 1; i <= 32; i++) {
        ...
        <div class="desc">
        ${codimgProjects[i].description}
        </div>
        <div class="links">
            <a href=${codimgProjects[i].link} target="_blank">Github</a>
        </div>
    </div>`;
    }
    animations();
